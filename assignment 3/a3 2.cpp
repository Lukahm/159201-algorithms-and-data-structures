...
int Queue::Length(){return counter;}
...
unsigned int sum_elements_array(int array[]){
	int sum=0;
	for(int a=0;a<N;a++){
		sum=sum+array[a];
	}
...
int main( int argc, char** argv ){//get arguments from command line, the name of the simulation text file
	...
	while(!input_file.eof()){
		...
		while(getline(line, token,' ')){
			...
			//Join values in Queue
			InputQueues[inputNumber].Join(destination); 
		...
		sumofinputpackets = InputQueues[inputNumber].Length();
	...
	while(currentsum>0){
		...
		packets = InputQueues[portnumber].Front();
		OutputQueues[packets-1].Join(packets);
		OutQueues_current[packets-1] = OutputQueues[packets-1].Length();
		...
		if(clock % (TIMEDELAY*number_of_ports) == 0 && clock!=0){ //DO NOT MODIFY THIS LINE!
			cout << "Packets can leave the output queues at " << clock << " microsec " << endl;
